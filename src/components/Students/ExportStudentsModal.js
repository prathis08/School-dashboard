...
import {
  useClasses,
  useStartStudentExport,
  useStudentExportStatus,
...
const ExportStudentsModal = ({ isOpen, onClose }) => {
  ...
  const startExport = useStartStudentExport();
  const { data: statusResponse } = useStudentExportStatus(jobId, {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      if (newJob?.jobId) {
        ...
        setErrorMessage("Could not start export. Please try again.");
      }
    } catch (err) {
  ...
  const handleDownload = async () => {
    if (!jobId) return;
    try {
      const url = studentsApi.getExportDownloadUrl(jobId);
      ...
      link.href = objectUrl;
      link.download = job?.filename || "students_export.xlsx";
      ...
      return (
        <>
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
            Choose what you want to export. The file will be prepared on the
            server and made available for download here.
          </p>
            ...
            <label className="flex items-start cursor-pointer p-3 border rounded-lg border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/40">
              ...
              <div>
                ...
                <div className="text-sm text-gray-500 dark:text-gray-400">
                  One workbook with a combined sheet plus a sheet per class.
                </div>
              </div>
            </label>
            ...
            <label className="flex items-start cursor-pointer p-3 border rounded-lg border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/40">
              ...
              <div className="flex-1">
                ...
                <div className="text-sm text-gray-500 dark:text-gray-400 mb-2">
                  Export only students assigned to a single class.
                </div>
                <select
                  value={classId}
        ...
        {isPreparing && (
          <div className="flex flex-col items-center text-center py-4">
            ...
            <div className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              {message || "The server is gathering student data."}
            </div>
          ...
          <div className="text-center py-4">
            ...
            <div className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              {job?.recordCount != null
                ? `${job.recordCount} student${job.recordCount === 1 ? "" : "s"} included.`
